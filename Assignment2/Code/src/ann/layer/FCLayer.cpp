...
{
    this->m_aWeights = xt::random::randn<double>({m_nNout, m_nNin}); // standard normal distribution
    this->m_aGrad_W = xt::zeros<double>({m_nNout, m_nNin});
...
xt::xarray<double> FCLayer::forward(xt::xarray<double> X)
{
    ...
    if (X.dimension() == 1)
    {
        auto temp = xt::view(X, xt::newaxis(), xt::all());
        Y = xt::linalg::dot(temp, xt::transpose(this->m_aWeights))[0];
    }
    else{
        Y = xt::linalg::dot(X, xt::transpose(this->m_aWeights));
    }
    if (this->m_bUse_Bias)
        Y += this->m_aBias;
    return Y;
...
xt::xarray<double> FCLayer::backward(xt::xarray<double> DY)
{
    ...
    int DY_shape = DY.shape()[0];
    for (int i = 0; i < DY_shape; i++)
        this->m_aGrad_W += xt::linalg::outer(xt::view(DY, i, xt::all()), xt::view(this->m_aCached_X, i, xt::all()));
    if (this->m_bUse_Bias)
    {
        for (int i = 0; i < DY_shape; i++)
            this->m_aGrad_b += xt::view(DY, i, xt::all());
    }    
