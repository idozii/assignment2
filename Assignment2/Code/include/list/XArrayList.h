...
XArrayList<T>::XArrayList(
    ...
    bool (*itemEqual)(T &, T &),
    int capacity)
    : capacity(capacity > 0 ? capacity : 10), count(0), itemEqual(itemEqual), deleteUserData(deleteUserData)
{
...
{
    // TODO
    if(index<0 || index>count-1) throw std::out_of_range("Index is out of range!");
    return data[index];
}
...
string XArrayList<T>::toString(string (*item2str)(T &))
{
    ...
    for(int i = 0; i<count; i++){
        if(i>0) ss << ", ";
        if(item2str != nullptr) ss << item2str(data[i]);
        else ss << data[i];
    }
    ss << "]";
    return ss.str();
